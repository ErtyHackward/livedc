...
namespace System.Windows
{
	...
	public class CollapsibleSplitter : System.Windows.Forms.Splitter
	{
        ...
        [Bindable(true), Category("Collapsing Options"), DefaultValue(true),
        Description("Allow user to resize handled control")]
...
Description("Reverse arrows")]
		...
		protected override void OnMouseDown(MouseEventArgs e)
		{
			...
			if(this.controlToHide!= null)
			{
				if(!this.hot && this.controlToHide.Visible && AllowResize)
				{
					base.OnMouseDown(e);
				}
			}
		...
		private Point[] ArrowPointArray(int x, int y)
		{
			...
			if(controlToHide!= null)
			{
				...
				if (
                    (this.Dock == DockStyle.Right && controlToHide.Visible ^ reversed)
                    || (this.Dock == DockStyle.Left && !controlToHide.Visible ^ reversed) 
					)
				{
					// right arrow
					...
					)
				{
                    ...
                    (this.Dock == DockStyle.Top && controlToHide.Visible ^ reversed)
                    || (this.Dock == DockStyle.Bottom && !controlToHide.Visible ^ reversed) 
					)
				{
					// up arrow
					point[0] = new Point(x + 3, y);
